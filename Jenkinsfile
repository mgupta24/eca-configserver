pipeline {
    environment {
        REGISTRY_NAME = "mgupta25/eca-configserver"
        ...
        stage('Sonar Analysis') {
                    steps {
                       sh "mvn -f $APP_NAME/pom.xml clean package"
                        dir("eca-configserver"){
                        sh "mvn sonar:sonar \
                        -Dsonar.projectKey=eca-configserver: \
                        -Dsonar.projectName=eca-configserver \
